...
**`move`** changes status, including closing. Say the status however you like, but a word that
only looks like a status name gets a question listing the seven, never a guess. Moving to
`backlog` or to `ready` also clears the
